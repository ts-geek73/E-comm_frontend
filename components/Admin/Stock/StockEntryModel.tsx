...
import { defultBasicForm, IBasicFormType, IStockEntry, ProductEntry, StockDetailsModalProps } from "@/types/stock"
...
const StockDetailsModal: React.FC<StockDetailsModalProps> = ({ open, onOpenChange, stockEntry, products, onSave, email }) => {
  ...
  const handleSave = () => {
    const validProductEntries = productEntries.filter((entry) => entry.product_id !== "" && entry.quantity !== 0)
    console.log("🚀 ~ handleSave ~ validProductEntries:", validProductEntries)
    console.log("🚀 ~ handleSave ~ formData:", formData)

    if (!formData.stock_name || !formData.description || validProductEntries.length === 0) {
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="min-w-4xl max-h-[90vh] overflow-auto">
          <DialogHeader>
            <DialogTitle>{isEditing ? "Stock Entry Details" : "New Stock Entry"}</DialogTitle>
            <DialogDescription>
              {isEditing
          ...
          <div className="grid gap-6 py-4">
            ...
            <ProductStockCard
              ...
              onUpdate={(productList) => console.log("ProductList", productList)}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave}>{isEditing ? "Update Entry" : "Create Entry"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
