...
import { ProductEntry } from "@/types/stock"
...
const ProductStockCard: React.FC<{
  ...
  const addProductEntry = useCallback(() => {
    const inValid = productEntries.filter((pe) => pe.product_id === "" || pe.quantity < 1)
    console.log("🚀 ~ addProductEntry ~ productEntries:", productEntries,inValid)
    if (inValid.length !== 0) {
