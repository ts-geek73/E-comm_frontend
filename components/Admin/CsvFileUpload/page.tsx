...
export const FileUpload = () => {
    ...
    const [loading, setLoading] = useState<LoadingState>({ parsing: false, uploading: false });
        ...
        if (fileExtension === "csv") {
            Papa.parse<RowData>(file, {
                ...
                complete: (result: ParseResult<RowData>) => {
                    ...
                    setData(result.data);
                    setShow(true);
                    setSuccess(`Successfully parsed ${result.data.length} rows from CSV file.`);
            ...
            reader.onload = (evt) => {
                try {
                    ...
                    setData(jsonData);
                    setShow(true);
                    setSuccess(`Successfully parsed ${jsonData.length} rows from Excel file.`);
    ...
    const handleFileImport = (fileData: File) => {
        ...
        apiServer.post("/product/file", formData, {
            headers: {
            ...
            .then((value) => {
                ...
                setSuccess(`Successfully uploaded and processed ${value.data.data.length} records.`);
            })
            .catch((error) => {
                setLoading(prev => ({ ...prev, uploading: false }));
