...
const PermissionModal: React.FC<Props> = ({ onClose, onSubmit, editingPermission }) => {
      ...
      <DialogContent className="sm:max-w-[450px]">
        ...
        <div className="grid gap-4 py-4">
          <Input
            ...
            onChange={(e) => setPermissionData((pre) => ({ ...pre, name: e.target.value } as Permission))}
            autoFocus
          />
          <Input
            placeholder="Permission Key"
            value={permissionData?.key}
            onChange={(e) => setPermissionData((pre) => ({ ...pre, key: e.target.value } as Permission))}
          />
          <Textarea
            placeholder="Description"
            value={permissionData?.description}
            onChange={(e) => setPermissionData((pre) => ({ ...pre, description: e.target.value } as Permission))}
            rows={4}
          />
        </div>
        <DialogFooter>
