...
import { ICArtProductPayLoad } from "@/types/product"
            ...
            {appliedCodes.length > 0 && (
                <div className="flex flex-wrap gap-2 pt-1">
                    {appliedCodes.map((code) => (
                        <span key={code} className="bg-green-100 text-green-700 text-xs px-3 py-1 rounded-full flex items-center">
                            {code}
                            <X
                        ...
                        <div className="space-y-1 text-sm">
                            ...
                            <ul className="list-disc ml-5 space-y-1">
                                {invalidCodes.map((item, idx) => (
                                    <li key={idx}><strong>{item.code}</strong> - {item.reason}</li>
                                ))}
                            </ul>
                        </div>
                        <button
...
export const renderCartItem = (item: ICArtProductPayLoad, rupeeSymbol:string, isOutOfStock = false) => (
    <div
        key={item._id}
        className={`flex items-center justify-between border-b border-gray-100 pb-3 ${isOutOfStock ? 'opacity-75' : ''
            }`}
    >
        <div className="flex items-center space-x-4">
            <div className={`relative h-16 w-16 overflow-hidden rounded-md border ${isOutOfStock ? 'border-red-200 bg-red-50' : 'border-gray-200'
                }`}>
                <Image
            ...
            <div>
                <p className={`font-medium text-sm ${isOutOfStock ? 'text-gray-500' : ''}`}>
                    {item.name}
                </p>
        ...
        <div className="text-right">
            <p className={`font-medium ${isOutOfStock ? 'text-red-400 line-through' : 'text-blue-500'}`}>
