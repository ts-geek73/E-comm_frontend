...
const MarkdownConverter = forwardRef<MarkdownConverterRef, MarkdownConverterProps>(
  ({ markdownText, onTocChange }, ref) => {
    ...
    const processedMarkdown = useMemo(() => {
      ...
      // Process badges
      const badgeRegex = /\[!\[([^\]]*)\]\[([^\]]+)\]\]\[([^\]]+)\]/g
      const withBadges = markdownText.replace(badgeRegex, (_, altText, badgeName, linkName) => {
        const badgeUrl = references[badgeName]
        const linkUrl = references[linkName]
        return badgeUrl && linkUrl ? `[![${altText}](${badgeUrl})](${linkUrl})` : _
      })
      ...
      <Markdown
        ...
        options={{
          overrides: {
              ...
              component: ({ children }: MDXComponentProps) => (
                <div className="my-8 relative overflow-hidden rounded-2xl border border-blue-200/50 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 p-6 shadow-lg backdrop-blur-sm">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="font-semibold text-blue-900 dark:text-blue-100">{children}</div>
                  </div>
                </div>
              ),
            },
              ...
              component: ({ question, children }: MDXComponentProps) => {
                return (
                  <div className="my-6 overflow-hidden rounded-xl border border-gray-200/50 bg-white/80 dark:border-gray-700/50 dark:bg-gray-800/80 shadow-lg backdrop-blur-sm">
                    ...
                    >
                      <span className="font-semibold text-gray-900 dark:text-gray-100 text-lg">{question}</span>
                      <div className={`transition-transform duration-200 ${isOpenAccordion ? "rotate-180" : ""}`}>
            ...
            tr: {
              component: ({ children }: MDXComponentProps) => (
                <tr className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-700/50">{children}</tr>
              ),
            },
            ...
            td: {
              component: ({ children }: MDXComponentProps) => (
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{children}</td>
              ),
            },
            ...
            blockquote: {
              component: ({ children }: MDXComponentProps) => (
                <blockquote className="relative rounded-2xl p-8 italic text-gray-700 dark:text-gray-200 shadow-lg bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-950/30 dark:to-pink-950/30 border-l-4 border-purple-400">
                  ...
                  <div className="relative z-10 text-lg leading-relaxed">{children}</div>
                </blockquote>
              ),
            },
            ...
            p: {
              component: ({ children }: MDXComponentProps) => (
                <div className="my-6 leading-8 text-gray-700 dark:text-gray-200 text-lg">{children}</div>
              ),
            },
            ...
            em: {
              component: ({ children }: MDXComponentProps) => (
                <em className="italic text-gray-700 dark:text-gray-200 font-medium">{children}</em>
              ),
            },

            del: {
              component: ({ children }: MDXComponentProps) => (
                <del className="line-through text-gray-500 dark:text-gray-400 opacity-75">{children}</del>
              ),
            },
