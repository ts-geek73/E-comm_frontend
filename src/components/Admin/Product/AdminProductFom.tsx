...
import {
  AdminFormProps,
  BrandCategory,
  ProductFormValues as FormValues,
  IBrand,
  ICategory,
} from "@types";
...
const ProductForm: React.FC<AdminFormProps> = ({
  ...
  const [previewImages, setPreviewImages] = useState<
    { url: string; isNew: boolean; file?: File }[]
  >([]);
    ...
    const fetchData = async () => {
      try {
        const { brands, categories } =
          (await getBrandsandCategories()) as BrandCategory;
  ...
  const removeImage = (index: number) => {
    ...
    if (removedImage.isNew) {
      ...
      const updatedImageUrls = currentImageUrls.filter(
        () =>
          index !==
          currentImageUrls.findIndex((img) => img.url === removedImage.url)
      );
    ...
    try {
      ...
      formData.append(
        ...
        JSON.stringify([
          {
            name:
              brands.find((b) => b._id === values.brand)?.name || values.brand,
            _id: values.brand,
          },
        ])
      );
                  ...
                  <FormItem className="w-full">
                    ...
                    <div className="flex items-center gap-2 max-w-md">
                      <Button
                        ...
                        onClick={() =>
                          field.onChange(
                            Math.max(Number(field.value) - 1000, 0)
                          )
                        }
                      >
                        –
                      ...
                      <Button
                        ...
                        // variant="secondary"
                        onClick={() =>
                          field.onChange(Number(field.value) + 1000)
                        }
                      >
                        +
                      </Button>
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <FormControl>
                          ...
                          >
                            {field.value
                              ? brands.find((b) => b._id === field.value)
                                  ?.name || "Select brand"
                              : "Select brand"}
                            <ChevronsUpDown className="ml-2 h-4 w-4 opacity-50" />
                          </Button>
