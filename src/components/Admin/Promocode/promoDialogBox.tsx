...
const PromoCodeDialog: React.FC<PromoCodeDialogProps> = ({
  open,
  onClose,
  promo,
  setPromo,
  ...
  const validate = () => {
    ...
    if (!["flat", "percentage"].includes(promo?.type ?? ""))
      newErrors.type = 'Type must be "flat" or "percentage"';
    if (!promo?.amount || promo.amount <= 0)
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {promo?._id ? "Edit Promo Code" : "Create Promo Code"}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div>
            <Input
              ...
              onChange={(e) =>
                setPromo((prev) => ({ ...prev!, expiryDate: e.target.value }))
              }
            />
            {errors.expiryDate && (
