...
import { Review, ReviewFecth } from "@types";
...
import {
  fetchReviews,
  handleDeleteReview,
} from "../Functions/review-whishlist";
...
const MultiReviewProduct = ({ productId }: { productId: string }) => {
  ...
  const avgRating: number = totalReviews.length
    ? totalReviews.reduce((sum, r: Review) => sum + r.rate, 0) /
      totalReviews.length
    : 0;
      ...
      <div>
        ...
        <div className="md:grid md:grid-cols-[1fr_2fr] gap-8 mt-6">
          <div className="">
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-xl text-center shadow-sm max-h-48 ">
              ...
              <p className="text-sm text-gray-600 mt-4 font-medium">
                Based on {reviewsObj.reviews.length}{" "}
                {reviewsObj.reviews.length === 1 ? "review" : "reviews"}
              </p>
            </div>
            <div>
              <Button
          ...
          <div className="mt-6 md:mt-0">
            {!isEditing ? (
              <>
                {reviewsObj.reviews.length + reviewsObj.otherReviews.length >
                0 ? (
                  <ReviewsList
