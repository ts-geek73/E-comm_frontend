import { AddressKey, ContactInfoFieldsProps, ExtendedFormValues } from "@types";
...
export function ContactInfoFields({
  ...
  const currentErrors = errors[
    fieldPrefix as AddressKey
  ] as FieldErrors<ExtendedFormValues>;

  return (
    <div className="grid grid-cols-2 gap-5">
      <div className="space-y-2">
        <Label htmlFor={`${fieldPrefix}-email`} className="text-blue-800">
          Email Address
        </Label>
        <Input
        ...
        {currentErrors?.email && (
          <span className="text-red-500 text-sm">
            {currentErrors?.email?.message as string}
          </span>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        {currentErrors?.phone && (
          <span className="text-red-500 text-sm">
            {currentErrors?.phone?.message as string}
          </span>
        )}
      </div>
    </div>
