      ...
      <CardContent className="space-y-4 pt-4">
        ...
        {cartdata?.inStock && cartdata.inStock.length > 0 && (
          <div className="space-y-3">
            ...
            {cartdata.inStock.map((item) =>
              renderCartItem(item, rupeeSymbol, false)
            )}
          </div>
        )}
        ...
        {cartdata?.outOfStock && cartdata.outOfStock.length > 0 && (
          <div className="space-y-3 mt-6">
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              ...
              <div className="space-y-3">
                {cartdata.outOfStock.map((item) =>
                  renderCartItem(item, rupeeSymbol, true)
                )}
              </div>
              <div className="mt-3 text-sm text-red-600 bg-red-100 p-2 rounded">
                <p className="font-medium">Note:</p>
                <p>
                  These items are currently out of stock and will not be
            ...
            {discountData && discountData.discountData?.length > 0 && (
              <>
                {discountData.discountData.map((d) => (
                  <div
                    key={d.code}
                    className="flex items-center justify-between text-green-600 text-sm"
                  >
