...
import { CheckoutFormProps, ExtendedFormValues, FormValues } from "@types";
...
export default function CheckoutForm({
  ...
  const [showNewBillingForm, setShowNewBillingForm] = useState(
    !savedAddresses.length
  );
  const [selectedShippingId, setSelectedShippingId] = useState<string | null>(
    null
  );
  const [showNewShippingForm, setShowNewShippingForm] = useState(
    !savedAddresses.length
  );
  ...
  useEffect(() => {
    if (sameAsBilling && billingData) {
      Object.keys(billingData).forEach((key) => {
        setValue(
          `shipping.${key}` as keyof ExtendedFormValues,
          billingData[key as keyof FormValues]
        );
      });
    }
  ...
  const handleAddNewAddress = async (
    ...
    toast.success(
      `${addressType === "billing" ? "Billing" : "Shipping"} address saved successfully!`
    );
  };
  ...
  return (
    <Card className="border-blue-100 shadow-lg">
      <CardHeader className="bg-blue-50 rounded-t-lg border-b border-blue-100">
        ...
        <CardDescription>
          Provide your billing and shipping details to finalize your order.
        </CardDescription>
      </CardHeader>
              ...
              <div className="space-y-4">
                <AddressForm
                  ...
                  onAddAddress={(address) =>
                    handleAddNewAddress(address, "billing")
                  }
                />
              </div>
            </TabsContent>
                ...
                {!sameAsBilling && (
                  <>
                    <AddressForm
                      ...
                      onAddAddress={(address) =>
                        handleAddNewAddress(address, "shipping")
                      }
                    />
                  </>
                )}
